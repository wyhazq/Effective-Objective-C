...
> #####1.[Objective-C起源](#1objective-c起源-1)
> #####2.[类.h文件尽量少引入其他类的.h文件](#2类h文件尽量少引入其他类的h文件-1)
> #####3.[使用字面量语法](#3使用字面量语法-1)
> #####4.[多用常量，少用宏定义](#4多用常量少用宏定义-1)
> #####5.[使用枚举表示状态、选项、状态码](#5使用枚举表示状态选项状态码-1)

二、[对象、消息、运行期](#二对象消息运行期-1)

> #####6.[Property](#6property-1)
> #####7.[对象内部尽量访问实例变量？](#7对象内部尽量访问实例变量-1)
> #####8.[对象比较](#8对象比较-1)
> #####9.[类族模式](#9类族模式-1)
> #####10.[使用关联对象存放自定义数据](#10使用关联对象存放自定义数据-1)
> #####11.[objc_msgSend](#11objc_msgsend-1)
> #####12.[消息转发 message forwarding](#12消息转发-message-forwarding-1)
> #####13.[method swizzling](#13method-swizzling-1)
> #####14.[类对象](#14类对象-1)

三、[接口与API设计](#三接口与API设计-1)

> #####15.[用前缀避免命名空间冲突](#15用前缀避免命名空间冲突-1)
> #####16.[提供全能初始化方法](#16提供全能初始化方法-1)
> #####17.[实现description方法](#17实现description方法-1)
> #####18.[尽量使用不可变对象](#18尽量使用不可变对象-1)
> #####19.[使用清晰而协调的命名方式](#19使用清晰而协调的命名方式-1)
> #####20.[为私有方法名加前缀](#20为私有方法名加前缀-1)
> #####21.[理解Objective-C错误类型](#21理解objective-c错误类型-1)
> #####22.[理解NSCopying协议](#22理解nscopying协议-1)

四、[协议与分类](#四协议与分类-1)

> #####23.[通过委托与数据源协议进行对象间通信](#23通过委托与数据源协议进行对象间通信-1)
> #####24.[将类的实现代码分散到便于管理的数个分类之中](#24将类的实现代码分散到便于管理的数个分类之中-1)
> #####25.[总是为第三方类的分类名称加前缀](#25总是为第三方类的分类名称加前缀-1)
> #####26.[勿在分类中声明属性](#26勿在分类中声明属性-1)
> #####27.[使用"class-continuation分类"隐藏实现细节](#27使用"class-continuation分类"隐藏实现细节-1)
> #####28.[通过协议提供匿名对象](#28通过协议提供匿名对象-1)

五、[内存管理](#五内存管理-1)

> #####29.[理解引用计数](#29理解引用计数-1)
> #####30.[以ARC简化引用计数](#30以arc简化引用计数-1)
> #####31.[在dealloc方法中只释放引用并解除监听](#31在dealloc方法中只释放引用并解除监听-1)
> #####32.[编写“异常安全代码”时留意内存管理问题](#32编写“异常安全代码”时留意内存管理问题-1)
> #####33.[以弱引用避免保留环](#33以弱引用避免保留环-1)
> #####34.[以"自动释放池块"降低内存峰值](#34以"自动释放池块"降低内存峰值-1)
> #####35.[用“僵尸对象”调试内存管理问题](#35用“僵尸对象”调试内存管理问题-1)
> #####36.[不要使用retainCount](#36不要使用retaincount-1)

六、[Block && GCD](#六block-&&-gcd-1)

> #####37.[理解Block](#37理解block)
> #####38.[为常用Block创建typedef](#38为常用block创建typedef-1)
> #####39.[用handler block降低代码分散程度](#39用handler-block降低代码分散程度-1)
> #####40.[用block引用其所属对象时不要出现保留环](#40用block引用其所属对象时不要出现保留环-1)
> #####41.[多用派发队列，少用同步锁](#41多用派发队列少用同步锁-1)
> #####42.[多用GCD，少用performSelector系列方法](#42多用gcd少用performselector系列方法-1)
> #####43.[掌握GCD及NSOperationQueue的使用时机](#43掌握gcd及nsoperationqueue的使用时机-1)
> #####44.[通过Dispatch Group机制，根据系统资源状况来执行任务](#44通过dispatch-group机制根据系统资源状况来执行任务-1)
> #####45.[使用dispatch_once来执行只需运行一次的线程安全代码](#45使用dispatch_once来执行只需运行一次的线程安全代码-1)
> #####46.[不要使用dispatch_get_current_queue](#46不要使用dispatch_get_current_queue-1)

七、[系统框架](#七系统框架-1)

> #####47.[熟悉系统框架](#47熟悉系统框架-1)
> #####48.[多用块枚举，少用for循环](#48多用块枚举少用for循环-1)
> #####49.[对自定义其内存管理语义的collection使用无缝桥接](#49对自定义其内存管理语义的collection使用无缝桥接-1)
> #####50.[构建缓存时选用NSCache而非NSDictionary](#50构建缓存时选用nscache而非nsdictionary-1)
> #####51.[精简initialize与load的实现代码](#51精简initialize与load的实现代码-1)
> #####52.[别忘了NSTimer会保留其目标对象](#52别忘了nstimer会保留期目标对象)
